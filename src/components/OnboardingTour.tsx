...
export function OnboardingTour() {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
            ...
            <div
                key={currentStep}
                className="relative w-full max-sm bg-white dark:bg-slate-900 rounded-[32px] overflow-hidden shadow-2xl z-10 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-300"
            >
                {/* Skip Button */}
                <button
                    onClick={finishTour}
                ...
                <div className={`h-32 w-full bg-gradient-to-br ${step.color} relative overflow-hidden flex items-center justify-center`}>
                    ...
                    <div
                        className="bg-white p-4 rounded-3xl shadow-lg transform rotate-3 z-10 relative animate-in zoom-in spin-in-12 duration-500 delay-100"
                    >
                        {step.icon}
                    </div>
                </div>
                ...
                <div className="p-8 text-center">
                    <h2
                        className="text-2xl font-black mb-3 text-slate-800 dark:text-slate-100 animate-in slide-in-from-bottom-2 duration-300 delay-200"
                    >
                        {step.title}
                    </h2>
                    <p
                        className="text-slate-500 dark:text-slate-400 font-medium text-[15px] leading-relaxed mb-8 h-16 animate-in slide-in-from-bottom-2 duration-300 delay-300"
                    >
                        {step.description}
                    </p>
