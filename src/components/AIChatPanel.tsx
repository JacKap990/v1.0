...
export function AIChatPanel({ isOpen, onClose }: AIChatPanelProps) {
    ...
    const handleTouchEnd = () => {
        if (touchStart !== null && touchMove !== null) {
            ...
            // Swipe right to close (as the panel is on the right)
            if (distance > 100) {
                onClose();
            }
        }
    ...
    const translateAmount = touchStart !== null && touchMove !== null
        ? Math.max(0, touchMove - touchStart)
        : 0;
    ...
    return (
        <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden">
            ...
            <div
                ...
                style={{
                    transform: translateAmount > 0 ? `translateX(${translateAmount}px)` : undefined,
                    transition: touchStart === null ? "transform 0.3s cubic-bezier(0.16, 1, 0.3, 1)" : "none"
                }}
                className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-left-0 sm:slide-in-from-right-full duration-300"
            >
                {/* Drag Handle (Mobile) */}
