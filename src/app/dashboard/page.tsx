...
export default function Dashboard() {
    ...
    const healthOffset = 251 - (251 * (Number(stats?.health) || 0)) / 100;
    ...
    return (
        <div className="p-6 space-y-8 pb-32 max-w-md mx-auto animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-2 gap-4">
                <button onClick={() => router.push("/inventory")} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center gap-3 active:scale-95 transition-all text-slate-800 hover:border-indigo-200 group">
                ...
                <div className="space-y-3">
                    {notifications.length > 0 ? (
                        notifications.map((note, idx) => (
                            <div
                                key={note.id || idx}
                                className={`p-4 rounded-3xl border transition-all animate-in slide-in-from-bottom-2 duration-300`}
                                style={{
                                    animationDelay: `${idx * 100}ms`,
                                    backgroundColor: note.severity === 'high' ? '#FFF1F2' : note.severity === 'medium' ? '#FFFBEB' : '#F5F3FF',
                                    borderColor: note.severity === 'high' ? '#FECDD3' : note.severity === 'medium' ? '#FDE68A' : '#DDD6FE'
                                }}
                            >
                        ...
                        <>
                            <div className="bg-slate-50 border border-slate-100 p-4 rounded-3xl flex items-center justify-between animate-in fade-in duration-500">
                                <div className="flex items-center gap-4">
                            ...
                            <div className="bg-slate-50/50 p-8 rounded-[40px] border border-dashed border-slate-200 text-center animate-in fade-in duration-500 delay-200">
