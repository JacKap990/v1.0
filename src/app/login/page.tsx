...
export default function LoginPage() {
    ...
    const handleForgotPassword = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/gateway/reset-password", {
                method: "POST",
    ...
    return (
        <div className="flex w-full h-full overflow-hidden flex-col items-center justify-center p-6 relative">
            ...
            {!showLogin ? (
                <div
                    key="onboarding"
                    className="glass w-full max-w-sm rounded-[32px] p-8 relative z-10 flex flex-col items-center text-center shadow-2xl border border-white/60 animate-in fade-in zoom-in-95 duration-400"
                >
                    {/* Interactive Carousel Area */}
                    <div className="h-[260px] flex flex-col items-center justify-center relative w-full mb-6">
                        <div
                            key={currentSlide}
                            className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-300"
                        >
                ...
                <div
                    key="login"
                    className="glass w-full max-w-sm rounded-[32px] p-8 relative z-10 flex flex-col items-center text-center shadow-2xl border border-white/60 animate-in fade-in slide-in-from-right-8 duration-400"
                >
                    {showForgotPassword ? (
                        <>
                            <button
